...
export const WA_MESSAGES = {
  trial: "Olá! Quero testar a Barber Agência (15 dias grátis). Pode me ajudar?",
  specialist: "Olá! Gostaria de falar com um especialista sobre a Barber Agência.",
  demo: (v: { name: string; phone: string; email: string }) =>
    `Olá! Quero uma demonstração da Barber Agência.\n\n` +
    `Barbearia: ${v.name || "-"}\nWhatsApp: ${v.phone || "-"}\nE-mail: ${v.email || "-"}`,
};
...
export const PLANS: Plan[] = [
  {
    ...
    features: [
      ...
      { ok: false, text: "Fila de atendimento com senha por QR/totem" },
  ...
  },
  {
    ...
    features: [
      ...
      { ok: true, text: "Fila de atendimento com senha por QR/totem" },
      { ok: false, text: "Emissão de nota fiscal via plataforma" },
      { ok: false, text: "Serviços de venda direta via plataforma" },
      { ok: false, text: "Gateway de pagamento via API / Webhooks" },
    ],
...
export const PLAN_TABLE: { label: string; a: string; b: string; c: string }[] = [
  ...
  { label: "Fila (senha por QR/totem)", a: "—", b: "Sim", c: "Sim" },
